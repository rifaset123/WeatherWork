...
class MyWorker(context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
    ...
    companion object {
        ...
        const val APP_ID = "f42dcb29000df64f9424fba5362d115d"
    ...
    // metode yang akan dipanggil ketika WorkManager berjalan
    override fun doWork(): Result {
        val dataCity = inputData.getString(EXTRA_CITY) // mengembalikan nilai untuk mengetahui status work
    ...
    private fun getCurrentWeather(city: String?): Result {
        ...
        Looper.prepare()
        val client = SyncHttpClient() // koneksi data yang sifatnya synchronous.
        //f42dcb29000df64f9424fba5362d115d
        val url = "https://api.openweathermap.org/data/2.5/weather?q=$city&appid=$APP_ID"

        // bagusan ditaruh di gradle buat security
        // val url = "https://api.openweathermap.org/data/2.5/weather?q=$city&appid=${BuildConfig.API_KEY}"

        Log.d(TAG, "getCurrentWeather: $url")
        client.post(url, object : AsyncHttpResponseHandler() { // koneksi secara asynchronous
